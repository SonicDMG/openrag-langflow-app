 ...
 *
 * @param searchTerm - Term to search for in filter names
 * @param client - Optional OpenRAGClient instance
...
export async function searchFilters(
  ...
  const useClient = client || new OpenRAGClient();
  return await useClient.knowledgeFilters.search(searchTerm);
}
...
export async function getFilter(
  ...
  const useClient = client || new OpenRAGClient();
  return await useClient.knowledgeFilters.get(filterId);
}
