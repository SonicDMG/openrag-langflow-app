...
describe('apiService - Query Structure Tests', () => {
  ...
  describe('fetchAvailableClasses', () => {
    it('should use the correct query structure with search context', async () => {
      ...
      await fetchAvailableClasses(mockAddLog, 'D&D');
      ...
      expect(requestBody.message).toContain('Using your tools, find all character sheets, details, and descriptions for heroes or classes that match "D&D"');
    ...
    it('should default to "D&D" when no search context provided', async () => {
      const mockResponse = {
        ok: true,
        body: {
          getReader: jest.fn(() => ({
      ...
      // Should default to "D&D"
      expect(requestBody.message).toContain('match "D&D"');
    });
  ...
  describe('Query Structure Validation', () => {
    it('should ensure fetchAvailableClasses query does not use old format', async () => {
      ...
      expect(requestBody.message).not.toContain('List all available D&D 5th edition character classes');
