...
interface ExportHeroesPDFCardProps {
  size?: 'normal' | 'compact';
}

export function ExportHeroesPDFCard({ size = 'compact' }: ExportHeroesPDFCardProps) {
  ...
  const handleExportPDF = async () => {
    ...
    try {
      // Fetch heroes data from API
      const response = await fetch('/api/heroes/export-pdf');

      if (!response.ok) {
        const data = await response.json();
        throw new Error(data.error || 'Failed to fetch heroes for PDF export');
      }
      ...
      if (!heroes || heroes.length === 0) {
        throw new Error('No heroes found to export');
      }

      // Convert heroes to PDF export format
      const heroesForPDF: CharacterPDFExportOptions[] = heroes.map((hero: any) => ({
        playerClass: {
          name: hero.name,
          ...
          color: hero.color,
          class: hero.class,
          meleeDamageDie: hero.meleeDamageDie,
          rangedDamageDie: hero.rangedDamageDie,
          race: hero.race,
          sex: hero.sex,
        },
        characterName: hero.name,
        // Use local CDN URL if available, otherwise fall back to imageUrl
        imageUrl: hero.localImageUrl || hero.imageUrl,
        characterType: hero.class ? `Hero - ${hero.class}` : 'Hero',
        // Use actual prompt from metadata if available
        imagePrompt: hero.imagePrompt,
        imageSetting: hero.imageSetting,
      }));

      // Generate the PDF
      const timestamp = new Date().toISOString().split('T')[0];
      const filename = `All_Heroes_${timestamp}.pdf`;
      
      await exportMultipleCharactersToPDF(heroesForPDF, {
        filename,
        footerText: '2025 OpenRAG - Battle Arena',
      });

      console.log(`Exported ${heroes.length} heroes to PDF: ${filename}`);
      setExportStatus('success');
      
      // Show success message
      alert(`✅ Successfully exported ${heroes.length} heroes to PDF!\n\nFilename: ${filename}\n\nThe PDF has been downloaded to your browser's download folder.`);
    } catch (error) {
      console.error('Failed to export heroes to PDF:', error);
      setExportStatus('error');
      alert(error instanceof Error ? error.message : 'Failed to export heroes to PDF. Please try again.');
    } finally {
    ...
    >
      ...
      <div className="text-center">
        <h3 className={`${titleSize} font-bold ${colors.title} mb-1`}>
          {isExporting ? 'Exporting...' : exportStatus === 'success' ? 'Exported!' : exportStatus === 'error' ? 'Export Failed' : 'Export Heroes to PDF'}
        </h3>
        <p className={`${textSize} ${colors.text}`}>
          {isExporting ? 'Generating PDF...' : exportStatus === 'success' ? 'Check your downloads' : exportStatus === 'error' ? 'Please try again' : 'Download all heroes'}
        </p>
      </div>
