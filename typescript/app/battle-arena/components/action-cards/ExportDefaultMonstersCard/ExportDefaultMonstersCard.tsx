...
interface ExportDefaultMonstersCardProps {
  size?: 'normal' | 'compact';
}

export function ExportDefaultMonstersCard({ size = 'compact' }: ExportDefaultMonstersCardProps) {
  ...
  const handleExportDefaults = async () => {
    ...
    try {
      const response = await fetch('/api/monsters/export-defaults', {
        method: 'POST',
      });

      if (!response.ok) {
        const data = await response.json();
        throw new Error(data.error || 'Failed to export default monsters');
      }

      const data = await response.json();
      console.log(`Exported ${data.count} default monsters to ${data.path}`);
      setExportStatus('success');
      
      // Show success message
      alert(`✅ Successfully exported ${data.count} default monsters to JSON!\n\nPath: ${data.path}\n\nRun 'npm run build' to use the updated defaults.`);
    } catch (error) {
      console.error('Failed to export default monsters:', error);
      setExportStatus('error');
      alert(error instanceof Error ? error.message : 'Failed to export default monsters. Please try again.');
    } finally {
    ...
    >
      ...
      <div className="text-center">
        <h3 className={`${titleSize} font-bold ${colors.title} mb-1`}>
          {isExporting ? 'Exporting...' : exportStatus === 'success' ? 'Exported!' : exportStatus === 'error' ? 'Export Failed' : 'Export Monsters to JSON'}
        </h3>
        <p className={`${textSize} ${colors.text}`}>
          {isExporting ? 'Saving to JSON file...' : exportStatus === 'success' ? 'Rebuild to use changes' : exportStatus === 'error' ? 'Please try again' : 'Save DB changes to file'}
        </p>
      </div>
