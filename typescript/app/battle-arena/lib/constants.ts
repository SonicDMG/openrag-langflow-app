import { Character, Ability, SettingConfig } from './types';
 ...
 *
 * Note: This function is less reliable than checking the _type marker on a Character object.
 * Prefer using (character as any)._type === 'monster' when you have the full object.
 *
 ...
 */
export function isMonster(className: string, availableMonsters?: Character[]): boolean {
...
// Note: abilities are included by default and will be randomly selected on each run
export const FALLBACK_MONSTERS: Character[] = FALLBACK_MONSTERS_DATA;
...
// Player Character Races (heroes/adventurers)
// These are races that can be player characters and heroes
// Includes standard hero races (Human, Elf, Dwarf, etc.) and "sometimes heroes"
// (Half-Orc, Tiefling, Drow, Yuan-ti) that can be heroes depending on setting
export const PLAYER_RACES: Array<{ name: string; description: string }> = [
...
/**
 * Randomly selects a player character race (for heroes/adventurers)
 */
export function getRandomRace(): { name: string; description: string } {
  return PLAYER_RACES[Math.floor(Math.random() * PLAYER_RACES.length)];
}
