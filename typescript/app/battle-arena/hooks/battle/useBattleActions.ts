...
export function useBattleActions(deps: BattleActionsDependencies) {
  ...
  const getAttackerClass = useCallback((attacker: 'player1' | 'player2' | 'support1' | 'support2'): Character | null => {
  ...
  const getDefenderClass = useCallback((target: 'player1' | 'player2' | 'support1' | 'support2'): Character | null => {
  ...
  const calculateAttackRoll = useCallback((attackerClass: Character): { d20Roll: number; attackRoll: number } => {
  ...
  const logAttackRoll = useCallback((attackerClass: Character, d20Roll: number, attackRoll: number, defenderAC: number, isHit: boolean) => {
  ...
  const performAttack = useCallback(async (attacker: 'player1' | 'player2' | 'support1' | 'support2', attackType?: 'melee' | 'ranged') => {
    ...
    let defenderClass: Character | null;
