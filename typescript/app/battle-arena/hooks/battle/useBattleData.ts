...
export function useBattleData() {
  ...
  const [availableClasses, setAvailableClasses] = useState<Character[]>([]);
  const [isLoadingClasses, setIsLoadingClasses] = useState(false);
  const [classesLoaded, setClassesLoaded] = useState(false);
  const [availableMonsters, setAvailableMonsters] = useState<Character[]>([]);
  ...
  const [createdMonsters, setCreatedMonsters] = useState<Array<Character & { monsterId: string; imageUrl: string }>>([]);
  ...
  useEffect(() => {
    ...
    try {
      const savedHeroes = localStorage.getItem('battle_arena_loaded_classes');
      const savedMonsters = localStorage.getItem('battle_arena_loaded_monsters');
      
      if (savedHeroes) {
        const heroes = JSON.parse(savedHeroes) as Character[];
      ...
      if (savedMonsters) {
        const monsters = JSON.parse(savedMonsters) as Character[];
    ...
    const loadClasses = async () => {
      ...
      try {
        ...
        if (heroes.length > 0) {
          ...
          localStorage.setItem('battle_arena_loaded_classes', JSON.stringify(heroes));
        } else {
    ...
    const loadMonsters = async () => {
      try {
        ...
        if (monsters.length > 0) {
          setAvailableMonsters(monsters);
          // Cache successful load for offline use
          localStorage.setItem('battle_arena_loaded_monsters', JSON.stringify(monsters));
        } else {
    ...
    const loadCreatedMonsters = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          // Convert created monsters to Character format
          const convertedMonsters = data.monsters.map((m: any) => {
            ...
            return {
              ...
              prompt: m.prompt,
              setting: m.setting,
            } as Character & { monsterId: string; imageUrl: string; imagePosition?: { offsetX: number; offsetY: number }; hasCutout?: boolean; lastAssociatedAt?: string; klass?: string; prompt?: string; setting?: string };
          });
