...
const MONSTERS_STORAGE_KEY = 'battle_arena_loaded_monsters';

/**
 * Load heroes from database, with fallback to localStorage
 * Updates localStorage on successful database load
 */
export async function loadHeroesFromDatabase(): Promise<Character[]> {
  try {
    ...
    try {
      ...
      if (savedHeroes) {
        const parsedHeroes = JSON.parse(savedHeroes) as Character[];
 ...
 * Updates localStorage on successful database load
 */
export async function loadMonstersFromDatabase(): Promise<Character[]> {
  try {
    ...
    if (response.ok) {
      ...
      if (data.monsters && data.monsters.length > 0) {
        ...
        const monstersWithType = data.monsters.map((m: Character) => ({
          ...m,
    ...
    try {
      ...
      if (savedMonsters) {
        const parsedMonsters = JSON.parse(savedMonsters) as Character[];
