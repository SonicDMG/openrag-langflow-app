 ...
 */
export async function loadDefaultHeroes(): Promise<Character[]> {
  try {
    ...
    return parsed.heroes as Character[];
  } catch (error) {
 ...
 */
export async function loadDefaultMonsters(): Promise<Character[]> {
  try {
    ...
    return parsed.monsters as Character[];
  } catch (error) {
 ...
 */
export function loadDefaultHeroesSync(): Character[] {
  try {
    const heroesPath = join(process.cwd(), '..', 'characters', 'default_heroes', 'heroes.json');
 ...
 * Note: This should only be used during build time or in server-side contexts
 */
export function loadDefaultMonstersSync(): Character[] {
  try {
    const monstersPath = join(process.cwd(), '..', 'characters', 'default_monsters', 'monsters.json');
